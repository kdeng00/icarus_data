...
public:
    ...
    Song retrieveRecord(const Song& song, Filter filter) {
        std::stringstream qry;
        auto conn = this->setup_connection();
        auto stmt = mysql_stmt_init(conn);
    ...
    bool doesSongExist(const Song& song, Filter filter) {
    ...
    bool deleteRecord(const Song& song) {
        auto conn = this->setup_connection();
        auto status = 0;
    ...
    void saveRecord(const Song& song) {
    ...
    void updateRecord(const Song& song) {
...
private:
    std::shared_ptr<MYSQL_BIND> valueBind(Song& song,
    ...
    std::vector<Song> parseRecords(MYSQL_STMT *stmt) {
        ::mysql_stmt_store_result(stmt);
